            ...
            try:
                    ...
                    if (ids is not None and len(ids) > 0) and all(
                        ...
                        rotation_vectors, translation_vectors, _ = (
                            my_estimatePoseSingleMarkers(
                                corners, marker_points, cameraMatrix, distCoeffs
                            )
                        )
                        self.preprocess_ids(ids, rotation_vectors, translation_vectors)

                        for rvec, tvec in zip(rotation_vectors, translation_vectors):
                            self.colorImage = aruco.drawDetectedMarkers(
                                self.colorImage, corners=corners, ids=ids
                            )
                            self.colorImage = cv2.drawFrameAxes(
                                self.colorImage,
                                cameraMatrix,
                                distCoeffs,
                                rvec,
                ...
                if self.inference == "YOLO":
                    yolo_results = model.predict(
                        self.colorImage, verbose=False, imgsz=640, conf=0.8
                    )[0]
